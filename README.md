...
- **Global hotkeys** — record (F5), play/stop (F6), capture cursor (F7), panic (F12);
...
> A plain debug `cargo build` points the webview at the Vite dev server, so the E2E
> app must be a production build (`--debug` keeps it fast; `--no-bundle` skips installers).
